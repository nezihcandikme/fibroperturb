...
export default function TargetOverview({language,targets,onSelect}:{language:Language;targets:TargetMetric[];onSelect:(name:string)=>void}){
 ...
 return <section className="target-overview" aria-label={t.title}>
  <div className="overview-head"><div><span className="eyebrow">DATASET / 1836</span><h3>{t.title}</h3><p>{t.text}</p></div><button className="overview-export" type="button" onClick={()=>exportTargetCsv(filtered)} disabled={!filtered.length}>{t.export} ↗</button></div>
